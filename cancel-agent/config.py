...
# ==========================================================
# Booking API (services/booking_service.py)
# ==========================================================
#
# NOTE: base_url is also carried per-client in client_config.csv
# ("base_url" column), since the n8n sub-workflows receive it as a
# workflow input on every call rather than reading a single global. When a
# client_id resolves to a config row, prefer that row's base_url over this
# global default (see graph/node wiring in tools.py).

BASE_URL: str = os.getenv(
    "BOOKING_API_BASE_URL",
    "https://demo.catalystsystems.io:1302",
)
...
def get_messages(client_id: str, dialect: Optional[str] = None) -> dict:
    ...
    merged["_base_url"] = client_row.get("base_url") or BASE_URL
